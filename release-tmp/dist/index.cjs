...
function IAvatar(props) {
  ...
  const [hasSourceError, setHasSourceError] = React9.useState(false);
  const [hasFallbackError, setHasFallbackError] = React9.useState(false);
  React9.useEffect(() => {
  ...
  const hostStyle = React9.useMemo(
...
function ICard(props) {
  ...
  const normalizedHref = React9.useMemo(() => normalizeHref(href), [href]);
  const toBase = React9.useMemo(() => routerLinkToTo(routerLink), [routerLink]);
  const search = React9.useMemo(() => buildSearch(queryParams), [queryParams]);
  const hash = React9.useMemo(() => buildHash(fragment), [fragment]);
  const to = React9.useMemo(() => {
...
function extractTextFromReactNode(node) {
  ...
  if (React9__default.default.isValidElement(node)) {
...
function ICodeViewer(props) {
  ...
  const hljsRef = React9.useRef(null);
  const hljsPromiseRef = React9.useRef(null);
  const shouldUseHljs = highlighter === "hljs" || highlighter === "auto";
  const loadHljsIfNeeded = React9.useCallback(async () => {
  ...
  const highlightWithHljs = React9.useCallback(
    (text, lang) => {
      var _a;
      try {
  ...
  const renderToHtmlSync = React9.useCallback(
    (text, lang) => {
      const raw = text ?? "";
      if (!raw) return "";
  ...
  const lineNumberList = React9.useMemo(() => {
    if (!lineNumbersC) return [];
    const lines = countLines(rawCode);
    return Array.from({ length: lines }, (_, i) => i + 1);
  }, [lineNumbersC, rawCode]);
  const [renderedHtml, setRenderedHtml] = React9.useState(
  ...
  const onCopy = React9.useCallback(async () => {
    var _a;
    const text = rawCode ?? "";
...
function useInputMask(inputRef, mask, opts = {}) {
  const defaultAppliedRef = React9.useRef(false);
...
var IInput = React9__default.default.forwardRef(
  function IInput2(props, forwardedRef) {
    ...
    const prepends = React9.useMemo(() => normalizeArray(prepend), [prepend]);
    const appends = React9.useMemo(
...
var ISelect = React9.forwardRef(function ISelectInner(props, ref) {
  const {
    placeholder = "",
    disabled = false,
    invalid = false,
  ...
  const rafRef = React9.useRef(0);
  const scrollParentsRef = React9.useRef([]);
  const listeningScrollParentsRef = React9.useRef(false);
  const [rawOptions, setRawOptions] = React9.useState(() => options ?? []);
  const [filteredOptions, setFilteredOptions] = React9.useState(
    () => options ?? []
  );
  const isControlled = value !== void 0;
  const [modelValue, setModelValue] = React9.useState(
    () => isControlled ? value ?? null : defaultValue ?? null
  );
  const [displayText, setDisplayText] = React9.useState("");
  const [filterText, setFilterText] = React9.useState("");
  const [isOpen, setIsOpen] = React9.useState(false);
  const [highlightIndex, setHighlightIndex] = React9.useState(-1);
  const [isLoading, setIsLoading] = React9.useState(false);
  const [panelHidden, setPanelHidden] = React9.useState(false);
  const wantsOpenRef = React9.useRef(false);
  const openSeqRef = React9.useRef(0);
  const unhideTimerRef = React9.useRef(null);
  const filterInput$ = React9.useMemo(() => new rxjs.Subject(), []);
  const filterSubRef = React9.useRef(null);
  const optionsSubRef = React9.useRef(null);
  React9.useEffect(() => {
    return () => {
  ...
  const panelPositionClass2 = React9.useMemo(
  ...
  const appendAddon = React9.useMemo(() => {
...
var IFCSelect = React9.forwardRef(function IFCSelectInner(props, ref) {
  const {
    label = "",
    placeholder = "",
    options = null,
  ...
  const innerSelectRef = React9.useRef(null);
  const focusInnerSelect = React9.useCallback(() => {
    var _a;
  ...
  const controlInvalid = React9.useMemo(() => {
  ...
  const required = React9.useMemo(
    () => isControlRequired({ errors: errors ?? void 0, errorMessage }),
    [errors, errorMessage]
  );
  const resolvedErrorText = React9.useMemo(
...
function IDatepicker(props) {
  ...
  const hostRef = React9.useRef(null);
  const panelRef = React9.useRef(null);
  const inputElRef = React9.useRef(null);
  const rafRef = React9.useRef(0);
  const scrollParentsRef = React9.useRef([]);
  const listeningScrollParentsRef = React9.useRef(false);
  const [modelValue, setModelValue] = React9.useState(null);
  const modelValueRef = React9.useRef(null);
  const [displayText, setDisplayText] = React9.useState("");
  const [isOpen, setIsOpen] = React9.useState(false);
  const [viewYear, setViewYear] = React9.useState(0);
  const [viewMonth, setViewMonth] = React9.useState(0);
  const [years, setYears] = React9.useState([]);
  const [panelHidden, setPanelHidden] = React9.useState(false);
  const wantsOpenRef = React9.useRef(false);
  const openSeqRef = React9.useRef(0);
  const unhideTimerRef = React9.useRef(null);
  const isEditingRef = React9.useRef(false);
  const lastEmittedKeyRef = React9.useRef(null);
  const pendingExternalRef = React9.useRef(void 0);
  const weeks = React9.useMemo(
    () => buildCalendar(viewYear, viewMonth, modelValue),
    [viewYear, viewMonth, modelValue]
  );
  const monthSelected = React9.useMemo(
    () => MONTHS.find((m) => m.value === viewMonth) ?? null,
    [viewMonth]
  );
  const dateMask = React9.useMemo(
    () => ({ type: "date", format }),
    [format]
  );
  React9.useEffect(() => {
  ...
  const refreshInnerInputRef = React9.useCallback(() => {
    var _a;
    const host = hostRef.current;
  ...
  const getAnchorEl = React9.useCallback(() => {
    const host = hostRef.current;
    if (!host) return null;
    const iInput = host.querySelector("i-input");
    return iInput ?? host;
  ...
  const syncFromInnerInputSafely = React9.useCallback(() => {
  ...
  const initViewFromModel = React9.useCallback(() => {
    let base;
  ...
  const repositionPanelNow = React9.useCallback(() => {
    var _a;
    if (!wantsOpenRef.current) return;
  ...
  const scheduleReposition = React9.useCallback(
  ...
  const addScrollParentListeners = React9.useCallback(() => {
    if (listeningScrollParentsRef.current) return;
    const anchor = getAnchorEl();
  ...
  const removeScrollParentListeners = React9.useCallback(() => {
    if (!listeningScrollParentsRef.current) return;
    const rm = addScrollParentListeners._rm;
    if (rm) rm();
    delete addScrollParentListeners._rm;
  ...
  const openPanel = React9.useCallback(() => {
    if (disabled) return;
    if (isOpen) return;
    setIsOpen(true);
  }, [disabled, isOpen]);
  const closePanel = React9.useCallback(
  ...
  const selectDay = React9.useCallback(
    (day) => {
      var _a;
      if (disabled) return;
  ...
  const prevMonth = React9.useCallback(() => {
    setViewMonth((m) => {
      if (m === 0) {
        setViewYear((y) => {
          const ny = y - 1;
  ...
  const nextMonth = React9.useCallback(() => {
    setViewMonth((m) => {
      if (m === 11) {
        setViewYear((y) => {
          const ny = y + 1;
  ...
  const onMonthChange = React9.useCallback(
    (change) => {
  ...
  const onYearChange = React9.useCallback(
    (change) => {
...
function ITextArea(props) {
  ...
  const handleHostClick = React9.useCallback(() => {
    var _a;
    if (!disabled) {
...
function IFCTextArea(props) {
  ...
  const hostRef = React9.useRef(null);
  const focusInnerTextarea = React9.useCallback(() => {
    if (disabled) return;
    const host = hostRef.current;
    if (!host) return;
...
var IDialogContext = React9.createContext(null);
var IDialogInstanceContext = React9.createContext(null);
function useIDialog() {
  const ctx = React9.useContext(IDialogContext);
  if (!ctx)
    throw new Error("useIDialog() must be used inside <IDialogProvider>.");
  return ctx;
}
function useIDialogData() {
  const ctx = React9.useContext(IDialogInstanceContext);
...
function IDialogProvider(props) {
  ...
  const [dialogs, setDialogs] = React9.useState([]);
  const open = React9.useCallback((component, config = {}) => {
  ...
  const closeById = React9.useCallback(
    (id, result) => {
  ...
  const closeAll = React9.useCallback(() => {
...
function IDialogContainer({
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("i-dialog-container", { class: className, role: "dialog", "aria-modal": "true", ...rest, children: [
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "i-dialog-backdrop", onClick: onBackdropClick }),
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "i-dialog-wrapper", children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "i-dialog-panel", style: panelStyles, children: instance && Comp ? /* @__PURE__ */ jsxRuntime.jsx(
...
function IDialogClose(props) {
  ...
  return React9__default.default.cloneElement(children, {
...
function IDialog(props) {
  ...
  const normalized = React9.useMemo(() => normalizeActions(actions), [actions]);
...
function IConfirm() {
  ...
  const [invalid, setInvalid] = React9.useState(false);
...
function useOptionalIConfirm() {
  const dialog = React9.useContext(IDialogContext);
  const show = React9.useCallback(
    async (data) => {
...
function normalizeTab(node, index) {
  if (!React9__default.default.isValidElement(node)) return null;
...
function ISectionTabs(props) {
  ...
  const tabs = React9.useMemo(() => {
    const arr = React9__default.default.Children.toArray(children);
    return arr.map((n, i) => normalizeTab(n, i)).filter(Boolean);
  }, [children]);
  const openedIndex = React9.useMemo(() => tabs.findIndex((t) => t.opened), [tabs]);
  const contentHeightPx = React9.useMemo(() => parseTabsHeight(height), [height]);
  ...
  const computeNextIndex = React9.useCallback(() => {
  ...
  const [activeIndex, setActiveIndex] = React9.useState(
    () => computeNextIndex()
  );
  const scrollContainerRef = React9.useRef(null);
  const [showLeftChevron, setShowLeftChevron] = React9.useState(false);
  const [showRightChevron, setShowRightChevron] = React9.useState(false);
  React9.useEffect(() => {
    setActiveIndex(computeNextIndex());
  }, [computeNextIndex]);
  const activeTab = tabs[activeIndex] ?? null;
  const setActive = React9.useCallback(
    (index, emit) => {
  ...
  const activateFromKeyboard = React9.useCallback(
    (event, index) => {
      var _a, _b;
  ...
  const scrollBy = React9.useCallback(
    (left) => {
      const container = scrollContainerRef.current;
...
function IErrorPage({
  ...
  const content = /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "i-error-page__content text-center", children: [
    resolved.icon || resolved.code ? /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "i-error-page__visual", children: [
      resolved.icon ? /* @__PURE__ */ jsxRuntime.jsx(IIcon, { icon: resolved.icon, size: "4xl", "aria-hidden": "true" }) : null,
      resolved.code ? /* @__PURE__ */ jsxRuntime.jsx("span", { className: "i-error-page__code font-bold leading-none", children: resolved.code }) : null
    ] }) : null,
    resolved.title ? /* @__PURE__ */ jsxRuntime.jsx("h1", { className: "m-0 text-3xl font-normal", children: resolved.title }) : null,
    /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "i-error-page__message text-md text-subtle leading-relaxed", children: [
      resolved.description ? /* @__PURE__ */ jsxRuntime.jsx("p", { className: "m-0 text-subtle leading-relaxed", children: resolved.description }) : null,
      /* @__PURE__ */ jsxRuntime.jsx("div", { className: "i-error-page__extra", children }),
      supportEmail ? /* @__PURE__ */ jsxRuntime.jsxs("p", { className: "m-0 text-subtle leading-relaxed", children: [
    ...
    /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "i-error-page__actions", children: [
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      ...
      ...rest,
      children: mode === "contained" ? /* @__PURE__ */ jsxRuntime.jsx(ISection, { className: "i-error-page__section", children: /* @__PURE__ */ jsxRuntime.jsx(ISectionBody, { className: "i-error-page__body", children: content }) }) : /* @__PURE__ */ jsxRuntime.jsx("div", { className: "i-error-page__body", children: content })
    }
  );
}
function IPaginator(props) {
  ...
  const pageItems = React9.useMemo(() => {
...
function IGridInner(props, ref) {
  ...
  const idRef = React9.useRef(Math.random().toString(36).slice(2));
  const selectionColumnWidth = 32;
  const numberColumnWidth = 60;
  const expandColumnWidth = 32;
  const defaultColumnWidth = 200;
  const numberColumn = React9.useMemo(
    () => ({
      fieldName: void 0,
      title: "No.",
      sortable: false,
  ...
  const [renderedData, setRenderedData] = React9.useState([]);
  const [currentFilterText, setCurrentFilterText] = React9.useState("");
  const [sortStates, setSortStates] = React9.useState([]);
  const columnWidthsRef = React9.useRef(/* @__PURE__ */ new Map());
  const [, setTick] = React9.useState(0);
  const [selectionSet, setSelectionSet] = React9.useState(/* @__PURE__ */ new Set());
  const [expandedSet, setExpandedSet] = React9.useState(/* @__PURE__ */ new Set());
  const selectionSetRef = React9.useRef(selectionSet);
  React9.useEffect(() => {
  ...
  const treeMetaRef = React9.useRef(/* @__PURE__ */ new Map());
  const treeRootsRef = React9.useRef([]);
  const { headerItems, columns, expandableRowDef } = React9.useMemo(() => {
    var _a;
    const all = React9.Children.toArray(children).filter(Boolean);
    let expandable = null;
    ...
    for (const node of all) {
      ...
      if (kind === "IGridColumnGroup") {
        const gp = node.props;
        const kids = React9.Children.toArray(gp.children).filter(
          Boolean
        );
        const gCols = [];
  ...
  const treeChildrenKey = React9.useMemo(() => {
  ...
  const sortStatesRef = React9.useRef([]);
  React9.useEffect(() => {
  ...
  const highlightSearchFn = React9.useMemo(
      ...
      effectiveHeaderItems.length ? /* @__PURE__ */ jsxRuntime.jsxs("i-grid-header-row", { class: "i-grid-header-row", children: [
        ...
        effectiveHeaderItems.map((item, idx) => {
          if (item.kind === "col") {
            ...
            if (col.headerDef) {
              return /* @__PURE__ */ jsxRuntime.jsx(React9__default.default.Fragment, { children: col.headerDef(col) }, `h-${idx}`);
            }
            return /* @__PURE__ */ jsxRuntime.jsx(
              HeaderCell,
              {
                ...
                /* @__PURE__ */ jsxRuntime.jsx(
                  "i-grid-header-cell-group-columns",
                  {
                    ...
                    children: item.columns.map((col, cIdx) => {
                      if (col.headerDef) {
                        return /* @__PURE__ */ jsxRuntime.jsx(React9__default.default.Fragment, { children: col.headerDef(col) }, `gc-${cIdx}`);
                      }
                      return /* @__PURE__ */ jsxRuntime.jsx(
                        HeaderCell,
                        {
...
var IGrid = React9.forwardRef(IGridInner);
var IAuthContext = React9.createContext(null);
function useIAuthContext() {
  const ctx = React9.useContext(IAuthContext);
  if (!ctx) {
    throw new Error(
...
function useISession() {
  ...
  React9.useSyncExternalStore(ctx.session.subscribe, ctx.session.getVersion);
  return ctx.session;
}
function useIApi() {
...
function useISessionExpired() {
  const ctx = useIAuthContext();
  React9.useSyncExternalStore(ctx.sessionExpired.subscribe, ctx.sessionExpired.getVersion);
  return ctx.sessionExpired;
}
function useIUserMenuStore() {
  const ctx = useIAuthContext();
  React9.useSyncExternalStore(ctx.userMenuStore.subscribe, ctx.userMenuStore.getVersion);
  return ctx.userMenuStore;
}
var IHostApiContext = React9.createContext(null);
...
function useHostApi() {
  const api = React9.useContext(IHostApiContext);
  if (!api)
    throw new Error(
...
function useHostApiOptional() {
  return React9.useContext(IHostApiContext);
}
var IHostUiContext = React9.createContext(null);
function IHostUiProvider(props) {
  const [title, setTitle] = React9.useState(null);
  const [breadcrumbs, setBreadcrumbs] = React9.useState(
    null
  );
...
function useHostUi() {
  const ctx = React9.useContext(IHostUiContext);
  if (!ctx)
...
var Highlighted = React9.memo(function Highlighted2(props) {
  const { text, term, as = "span" } = props;
  const html = React9.useMemo(() => highlightSearchHtml(text, term), [text, term]);
  const Tag = as;
...
function IHContent(props) {
  ...
  const onCrumbClick = React9.useCallback(
    (e, url) => {
...
var IHMenu = React9.memo(function IHMenu2(props) {
  const {
    menu,
    filter,
    selectedMenuId,
  ...
  const menuKey = React9.useMemo(() => getMenuKey(menu), [menu]);
  const menuLabel = React9.useMemo(() => getMenuLabel(menu), [menu]);
  const menuRoute = React9.useMemo(() => getMenuRoute(menu), [menu]);
  const hasChild = React9.useMemo(() => hasMenuChildren(menu), [menu]);
  const menuChildren = React9.useMemo(() => getMenuChildren(menu), [menu]);
  const isModuleNode = React9.useMemo(() => isModuleMenu(menu), [menu]);
  const isGroupNodeValue = React9.useMemo(() => isGroupNode(menu), [menu]);
  const isLeaf = React9.useMemo(() => isLeafItem(menu), [menu]);
  ...
  const favoriteSubtitle = React9.useMemo(() => {
    var _a;
  ...
  const iconClass = React9.useMemo(() => resolveMenuIcon(menu == null ? void 0 : menu.icon), [menu]);
  const isGroupExpanded = React9.useMemo(() => {
  ...
  const isSelected = React9.useMemo(() => {
    if (!menu) return false;
  ...
  const clickGroup = React9.useCallback(() => {
    if (!menu) return;
  ...
  const onLeafClick = React9.useCallback(
    (e) => {
      if (!menu) return;
      if (!menuRoute) return;
      if (!isPlainLeftClick(e)) return;
  ...
  const onToggleFavorite = React9.useCallback(
    async (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (menuKey === null) return;
  ...
  const renderLeafInner = () => /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    renderIndent(),
    ...
    /* @__PURE__ */ jsxRuntime.jsxs(
      "span",
      {
        ...
        children: [
          ...
          favoriteSubtitle ? /* @__PURE__ */ jsxRuntime.jsx("small", { className: "ih-menu-application", children: favoriteSubtitle }) : null
        ]
      }
    ),
...
function FavoritesSection({
  ...
  const [favoritesCollapsed, setFavoritesCollapsed] = React9.useState(false);
  const [favoriteItems, setFavoriteItems] = React9.useState(favorites);
  const [dragOver, setDragOver] = React9.useState(false);
  const listRef = React9.useRef(null);
  const itemsRef = React9.useRef(favorites);
  const dragRef = React9.useRef(null);
  const dragHandlersRef = React9.useRef(null);
  React9.useEffect(() => {
    itemsRef.current = favorites;
    setFavoriteItems(favorites);
  }, [favorites]);
  const pathByKey = React9.useMemo(
    () => buildFavoritePathMap(menus, favoriteItems),
    [menus, favoriteItems]
  );
  const group = React9.useMemo(() => {
  ...
  const computeDropIndex = React9.useCallback((clientY) => {
  ...
  const reorderLive = React9.useCallback((menuId, targetIndex) => {
  ...
  const cleanupDrag = React9.useCallback(() => {
    var _a, _b;
  ...
  const startDrag = React9.useCallback(
    (event) => {
      const target = event.target;
...
function IHSidebar(props) {
  ...
  const navigate = reactRouterDom.useNavigate();
  const auth = React9.useContext(IAuthContext);
  const headerRef = React9.useRef(null);
  const [accountMenuOpen, setAccountMenuOpen] = React9.useState(false);
  ...
  const onLogoutClick = React9.useCallback(() => {
    var _a, _b;
    setAccountMenuOpen(false);
    if (!auth) return;
  ...
  const initialFilter = React9.useMemo(() => {
    const sp = new URLSearchParams(location.search);
    return sp.get("menu-filter") ?? "";
  }, [location.search]);
  const [menuFilter, setMenuFilter] = React9.useState(initialFilter);
  const [keyboardNavActive, setKeyboardNavActive] = React9.useState(false);
  const [selectedIndex, setSelectedIndex] = React9.useState(null);
  const [selectedMenuId, setSelectedMenuId] = React9.useState(null);
  const [menuTree, setMenuTree] = React9.useState(() => normalizeMenuTree(menus));
  React9.useEffect(() => {
    setMenuTree(normalizeMenuTree(menus));
  }, [menus]);
  const filteredMenus = React9.useMemo(
    () => filterMenuTree(menuTree, menuFilter),
    [menuTree, menuFilter]
  );
  const navigableMenus = React9.useMemo(
    () => flattenNavigableMenus(filteredMenus),
    [filteredMenus]
  );
  const updateUrl = React9.useCallback(
    (nextFilter) => {
      const sp = new URLSearchParams(location.search);
  ...
  const onFilterChange = React9.useCallback(
    (next) => {
      setMenuFilter(next);
      setKeyboardNavActive(false);
      setSelectedIndex(null);
  ...
  const navigateToMenu = React9.useCallback(
    (menu) => {
      const route = getMenuRoute(menu);
      if (!route) return;
  ...
  const onSearchKeyDown = React9.useCallback(
    (event) => {
  ...
  const onToggleGroup = React9.useCallback((menuId) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("ih-sidebar", { class: !visible ? "hidden" : void 0, children: [
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "ih-sidebar-header", ref: headerRef, children: user ? /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
      /* @__PURE__ */ jsxRuntime.jsxs(
        "button",
        {
          type: "button",
          ...
          children: [
            /* @__PURE__ */ jsxRuntime.jsx("span", { className: "user-image", children: /* @__PURE__ */ jsxRuntime.jsx(IAvatar, { alt: user.fullName, size: 28, src: user.userImagePath }) }),
            /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "user-info", children: [
      ...
      accountMenuOpen ? /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "ih-user-dropdown i-options", role: "menu", children: [
        /* @__PURE__ */ jsxRuntime.jsxs(
          "a",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntime.jsx("i", { className: "fa-solid fa-user fa-fw" }),
              /* @__PURE__ */ jsxRuntime.jsx("span", { className: "i-option-label", children: "Personal Profile" })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntime.jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntime.jsx("i", { className: "fa-solid fa-right-from-bracket fa-fw" }),
              /* @__PURE__ */ jsxRuntime.jsx("span", { className: "i-option-label", children: "Logout" })
            ]
          }
        )
      ] }) : null
...
var IRouteAccessContext = React9.createContext(null);
...
function IRouter(props) {
  const {
    ...
    notFound = /* @__PURE__ */ jsxRuntime.jsx(IErrorPage, { kind: "not-found" })
  } = props;
  const hostApi = useHostApiOptional();
  const location = reactRouterDom.useLocation();
  const [deniedOwners, setDeniedOwners] = React9.useState(() => /* @__PURE__ */ new Set());
  const reportAccess = React9.useCallback((owner, denied2) => {
  ...
  const expandedRoutes = React9.useMemo(
    () => expandImplicitIndexRoutes(routes),
    [routes]
  );
  const routeElements = React9.useMemo(
    () => renderRouteTree(expandedRoutes),
    [expandedRoutes]
  );
  const lastTitleRef = React9.useRef(null);
  const lastCrumbsKeyRef = React9.useRef("");
  React9.useEffect(() => {
    var _a;
    if (!hostApi) return;
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(IRouteAccessContext.Provider, { value: reportAccess, children: /* @__PURE__ */ jsxRuntime.jsx(React9.Suspense, { fallback: loading, children: /* @__PURE__ */ jsxRuntime.jsxs(reactRouterDom.Routes, { children: [
    routeElements,
...
function IToggle(props) {
  ...
  const [uncontrolledChecked, setUncontrolledChecked] = React9.useState(defaultChecked);
  const currentChecked = isControlled ? !!checked : uncontrolledChecked;
  const hostClassName = React9.useMemo(() => {
    return [
      "i-toggle",
  ...
  const sizeStyle = React9.useMemo(() => {
  ...
  const emitChange = React9.useCallback(
    (next) => {
  ...
  const handleNativeChange = React9.useCallback(
    (e) => {
      if (disabled) return;
...
function IAuthCallback() {
  ...
  const location = reactRouterDom.useLocation();
  const navigate = reactRouterDom.useNavigate();
  const [handled, setHandled] = React9.useState(false);
  React9.useEffect(() => {
    if (handled) return;
    setHandled(true);
...
function IAuthProvider(props) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(IAuthContext.Provider, { value: runtime, children: props.children });
}
    ...
    IDialogContainer,
    {
      ...
      children: /* @__PURE__ */ jsxRuntime.jsx(
        IDialog,
        {
          ...
          children: /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex flex-col align-center text-center gap-lg", children: [
            /* @__PURE__ */ jsxRuntime.jsx(IIcon, { className: "text-warning", icon: iconClass, size: "2xl" }),
            /* @__PURE__ */ jsxRuntime.jsx("h4", { className: "font-semibold", children: TITLES[reason ?? "default"] }),
            /* @__PURE__ */ jsxRuntime.jsx("p", { className: "m-0 text-md leading-normal text-subtle", children: message })
          ] })
        }
      )
    }
...
function IRequireAccess({
  ...
  const [accessLoading, setAccessLoading] = React9.useState(
  ...
  if (source === "menuCode" && (store.initializing || !accessSettled || accessLoading)) {
    return loading ?? /* @__PURE__ */ jsxRuntime.jsx("div", { className: "ih-route-loading", children: "Loading access..." });
  }
  const allowed = source === "role" ? session.hasRole(value) : store.hasMenuCode(value);
  if (!allowed) {
    return /* @__PURE__ */ jsxRuntime.jsx(reactRouterDom.Navigate, { to: unauthorizedPath, replace: true });
  }
  return /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children });
}
function IRequireRouteAccess({
  ...
  if (!resolvedMenuCode) {
    ...
    return missingMenuCode === "allow" ? /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children }) : /* @__PURE__ */ jsxRuntime.jsx(reactRouterDom.Navigate, { to: unauthorizedPath, replace: true });
  }
...
function IHasMnRoute({ value, children }) {
  ...
  const store = useIUserMenuStore();
  const allowed = usePermission(value);
  const reportAccess = React9.useContext(IRouteAccessContext);
  const owner = React9.useId();
  ...
  if (!ready) {
    return /* @__PURE__ */ jsxRuntime.jsx(ILoading, { label: "Loading access...", "aria-live": "polite" });
  }
  if (!allowed) {
    return /* @__PURE__ */ jsxRuntime.jsx(IErrorPage, { kind: "forbidden", role: "alert" });
  }
  return /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children });
}
function hasMn(value, content) {
  if (typeof content === "function") {
    const Page = React9.lazy(async () => ({ default: await content() }));
    const GuardedPage = () => /* @__PURE__ */ jsxRuntime.jsx(IHasMnRoute, { value, children: /* @__PURE__ */ jsxRuntime.jsx(React9.Suspense, { fallback: /* @__PURE__ */ jsxRuntime.jsx(ILoading, { label: "Loading page...", "aria-live": "polite" }), children: /* @__PURE__ */ jsxRuntime.jsx(Page, {}) }) });
    return async () => GuardedPage;
  }
  return React9.createElement(IHasMnRoute, { value, children: content });
}

exports.DEFAULT_ERROR_FACTORIES = DEFAULT_ERROR_FACTORIES;
exports.DEFAULT_PERSONAL_PROFILE_URL = DEFAULT_PERSONAL_PROFILE_URL;
exports.HostShell = HostShell;
exports.IAlert = IAlert;
exports.IAuthCallback = IAuthCallback;
exports.IAuthContext = IAuthContext;
exports.IAuthProvider = IAuthProvider;
...
exports.IHasMn = IHasMn;
exports.IHasMnRoute = IHasMnRoute;
...
exports.INotHasMn = INotHasMn;
exports.IPaginator = IPaginator;
exports.IPill = IPill;
exports.IRequireAccess = IRequireAccess;
exports.IRequireAuth = IRequireAuth;
exports.IRequireRouteAccess = IRequireRouteAccess;
...
exports.ISessionExpiredDialog = ISessionExpiredDialog;
exports.ISessionExpiredService = ISessionExpiredService;
...
exports.IUserMenuService = IUserMenuService;
exports.IUserMenuStore = IUserMenuStore;
exports.I_ERROR_PAGE_ACTIONS = I_ERROR_PAGE_ACTIONS;
exports.I_ERROR_PAGE_PRESETS = I_ERROR_PAGE_PRESETS;
exports.I_ERROR_PAGE_SUPPORT_EMAIL = I_ERROR_PAGE_SUPPORT_EMAIL;
exports.I_ICON_NAMES = I_ICON_NAMES;
exports.I_ICON_SIZES = I_ICON_SIZES;
exports.UNAUTHORIZED_ACCESS_PATH = UNAUTHORIZED_ACCESS_PATH;
exports.USER_APPLICATION_MAPPING_NOT_FOUND = USER_APPLICATION_MAPPING_NOT_FOUND;
exports.asMinMaxLengthError = asMinMaxLengthError;
exports.buildExternalSigninUrl = buildExternalSigninUrl;
exports.buildFavoritePathMap = buildFavoritePathMap;
exports.buildUrl = buildUrl2;
exports.collectAuthorizationScope = collectAuthorizationScope;
exports.collectLeafRoutes = collectLeafRoutes;
exports.collectMenuChain = collectMenuChain;
exports.collectMenuCodes = collectMenuCodes;
exports.createApiClient = createApiClient;
exports.createIRuntime = createIRuntime;
...
exports.formatApiFieldErrors = formatApiFieldErrors;
exports.getAuthEndpointPath = getAuthEndpointPath;
exports.getAuthEndpointUrl = getAuthEndpointUrl;
exports.getDefaultIAuthConfig = getDefaultIAuthConfig;
exports.getDefaultIAuthEndpoints = getDefaultIAuthEndpoints;
...
exports.hasAnyRoute = hasAnyRoute;
...
exports.requireIdentityHost = requireIdentityHost;
exports.resolveApiErrorDisplayMessage = resolveApiErrorDisplayMessage;
exports.resolveControlErrorMessage = resolveControlErrorMessage;
exports.resolveIAuthConfig = resolveIAuthConfig;
...
exports.useIApi = useIApi;
exports.useIAuth = useIAuth;
exports.useIAuthContext = useIAuthContext;
exports.useIConfirm = useIConfirm;
exports.useICsrf = useICsrf;
exports.useIDialog = useIDialog;
exports.useIDialogData = useIDialogData;
exports.useIDialogRef = useIDialogRef;
exports.useISession = useISession;
exports.useISessionExpired = useISessionExpired;
exports.useIUserMenuStore = useIUserMenuStore;
exports.useInputMask = useInputMask;
exports.useOptionalIConfirm = useOptionalIConfirm;
exports.usePermission = usePermission;
exports.validateIAuthConfig = validateIAuthConfig;
