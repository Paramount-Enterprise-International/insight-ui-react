...
function ICard(props) {
  ...
  const effectiveHref = React8.useMemo(
...
function IDatepicker(props) {
  ...
  const rafRef = React8.useRef(0);
  const listeningGlobalRef = React8.useRef(false);
  ...
  const addGlobalListeners = React8.useCallback(() => {
  ...
  const removeGlobalListeners = React8.useCallback(() => {
    if (!listeningGlobalRef.current) return;
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "i-datepicker",
    {
      ...
      children: [
        ...
        portalToBody ? reactDom.createPortal(panelNode, document.body) : panelNode
      ]
    }
  );
}
