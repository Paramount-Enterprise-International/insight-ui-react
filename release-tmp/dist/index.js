...
var InsightAuthContext = createContext(null);
...
function IHSidebar(props) {
  ...
  return /* @__PURE__ */ jsxs("ih-sidebar", { class: !visible ? "hidden" : void 0, children: [
    /* @__PURE__ */ jsx("div", { className: "ih-sidebar-header", children: user ? /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsx("div", { className: "user-image", children: /* @__PURE__ */ jsx(IAvatar, { alt: user.fullName, size: 28, src: user.userImagePath }) }),
...
function requireIdentityHost(config) {
  if (!config.api.identity) {
    throw new Error(
      `[@insight/ui-react] api.identity is not configured. Point it at this app's own auth host/BFF (e.g. <InsightAuthProvider config={{ api: { identity: "https://<your-app>/api" }, signinUrl: "..." }} />).`
    );
  }
...
function getAuthEndpointPath(config, key) {
  var _a;
  return ((_a = config.endpoints) == null ? void 0 : _a[key]) ?? getDefaultInsightAuthEndpoints()[key] ?? "";
}
...
function validateInsightAuthConfig(config) {
  if (!config.api.identity) {
    throw new Error(
      "[@insight/ui-react] InsightAuthProvider requires api.identity - the base URL of this app's own auth host/BFF. The library no longer defaults to a shared identity provider."
    );
  }
  if (!config.signinUrl) {
    throw new Error(
      "[@insight/ui-react] InsightAuthProvider requires signinUrl - the full URL of this app's sign-in page / BFF login route."
    );
  }
}
...
function InsightAuthProvider({
  ...
  return /* @__PURE__ */ jsx(InsightAuthContext.Provider, { value: services.value, children });
}
...
export { AuthCallback, AuthService, CsrfService, CurrentUserService, DEFAULT_ERROR_FACTORIES, HasMn, HostShell, IAlert, IAvatar, IButton, ICard, ICardBody, ICardFooter, ICardImage, ICodeViewer, IConfirm, IDatepicker, IDialog, IDialogClose, IDialogOutlet, IDialogProvider, IDialogRef, IFCDatepicker, IFCInput, IFCSelect, IFCTextArea, IGrid, IGridColumn, IGridColumnGroup, IGridCustomColumn, IGridDataSource, IGridExpandableRow, IHContent, IHContentLayout, IHMenu, IHSidebar, IHostApiProvider, IHostUiProvider, IIcon, IInput, IInputAddon, ILoading, IPaginator, IPill, IRouter, ISection, ISectionBody, ISectionFilter, ISectionFooter, ISectionHeader, ISectionSubHeader, ISectionTab, ISectionTabs, ISelect, ITextArea, IToggle, I_ICON_NAMES, I_ICON_SIZES, InsightAuthContext, InsightAuthProvider, NotHasMn, RequireAuth, SessionExpiredDialog, SessionExpiredService, SessionService, StorageService, USER_APPLICATION_MAPPING_NOT_FOUND, UserMenuService, UserMenuStore, asMinMaxLengthError, buildExternalSigninUrl, buildFavoritePathMap, buildUrl2 as buildUrl, collectMenuChain, collectMenuCodes, createApiClient, decodeJwtPayload, decodeUser, environment, extractAccessTokenFromHash, extractProblemDetailsErrorCode, findFirstLeafRoute, findMenuNameById, getAuthEndpointPath, getAuthEndpointUrl, getDefaultInsightAuthConfig, getDefaultInsightAuthEndpoints, getMenuChildren, getMenuKey, getMenuLabel, getMenuRoute, hasAnyMenuCode, hasMenuChildren, hasNumber, interpolate, isControlRequired, isGroupNode, isHttpRoute, isLeafItem, isModuleMenu, isNewTabMenu, isRecord, isReloadMenu, isSessionExpiredError, isSpaMenu, mapToSidebarUser, normalizeApiError, normalizeFetchError, normalizeMenuTree, rawRequest, readNumber, requireIdentityHost, resolveApiErrorDisplayMessage, resolveControlErrorMessage, resolveInsightAuthConfig, resolvePermission, sanitizeReturnUrl, sessionExpiredService, storageService, toIMenu, toIMenuFavorite, toIMenus, toSessionExpiredReason, useApi, useAuth, useCsrf, useHostApi, useHostApiOptional, useHostUi, useIAlert, useIConfirm, useIDialog, useIDialogData, useIDialogRef, useInputMask, useInsightAuth, useOptionalIConfirm, usePermission, useSession, useSessionExpired, useUserMenuStore, validateInsightAuthConfig };
