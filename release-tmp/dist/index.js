...
import React8, { useRef, useMemo, createContext, memo, useLayoutEffect, useCallback, useEffect, useState, useContext, Children, Suspense, useId, lazy } from 'react';
...
function useInputMask(inputRef, mask, opts = {}) {
  ...
  useEffect(() => {
    ...
    const setValue = (v, opts2) => {
      const old = el.value ?? "";
      if (v === old) return;
      const prevPos = (opts2 == null ? void 0 : opts2.anchorPos) ?? el.selectionStart ?? old.length;
      el.value = v;
      ...
      if ((opts2 == null ? void 0 : opts2.emit) !== false) {
        dispatchInput();
      }
    };
...
function IDatepicker(props) {
  ...
  useEffect(() => {
    ...
    const base = next ?? parseInputDate(next ? formatDateLocal(next, format) : "", format) ?? startOfDay(/* @__PURE__ */ new Date());
