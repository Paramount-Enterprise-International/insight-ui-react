...
import React__default, { ReactNode, HTMLAttributes, ReactElement, JSX } from 'react';
...
type IDialogContainerProps = React__default.HTMLAttributes<HTMLElement> & {
...
declare function IDialogContainer({ instance, config, isTopMost, onClose, children, className, ...rest }: IDialogContainerProps): React__default.JSX.Element;
...
/** Built-in recovery actions reported by the error page. */
...
type IErrorPageProps = HTMLAttributes<HTMLDivElement> & {
...
declare function IErrorPage({ kind, mode, title, description, icon, code, supportEmail, actions, onAction, children, customActions, className, ...rest }: IErrorPageProps): ReactElement;
...
type IFCInputProps = Omit<React__default.HTMLAttributes<HTMLElement>, 'children' | 'onInput' | 'onBlur'> & {
    label?: string;
    placeholder?: string;
    autocomplete?: string;
    readonly?: boolean;
...
declare function resolveApiErrorDisplayMessage(error: unknown, localFallback: string, catalogResolver?: IApiErrorCatalogResolver, formatter?: IApiErrorDisplayFormatter): string;
...
type IResponseType = 'json' | 'response' | 'blob' | 'arraybuffer' | 'text';
/** Query parameters accepted as a plain object or URLSearchParams. */
type IQueryParams = Record<string, string | number | boolean | undefined> | URLSearchParams;
/** Shared transport options accepted by every API method. */
type IApiOptions<R extends IResponseType = 'json'> = {
    apiUrl?: string;
...
type IRequestOptions<R extends IResponseType = 'json'> = IApiOptions<R> & {
...
declare function rawRequest(baseUrl: string, path: string, csrf: ICsrfService | null, options: IRequestOptions<'response'> & {
    responseType: 'response';
}): Promise<Response>;
declare function rawRequest(baseUrl: string, path: string, csrf: ICsrfService | null, options: IRequestOptions<'blob'> & {
    responseType: 'blob';
}): Promise<Blob>;
declare function rawRequest(baseUrl: string, path: string, csrf: ICsrfService | null, options: IRequestOptions<'arraybuffer'> & {
    responseType: 'arraybuffer';
}): Promise<ArrayBuffer>;
declare function rawRequest(baseUrl: string, path: string, csrf: ICsrfService | null, options: IRequestOptions<'text'> & {
    responseType: 'text';
}): Promise<string>;
declare function rawRequest<T = unknown>(baseUrl: string, path: string, csrf: ICsrfService | null, options?: IRequestOptions & {
    responseType?: 'json';
}): Promise<T>;
/** Combines a base URL, path, and optional query parameters. */
declare function buildUrl(baseUrl: string, path: string, params?: IQueryParams): string;
...
interface IApiRead {
    (path: string, options: IApiOptions<'response'> & {
        responseType: 'response';
    }): Promise<Response>;
    (path: string, options: IApiOptions<'blob'> & {
        responseType: 'blob';
    }): Promise<Blob>;
    (path: string, options: IApiOptions<'arraybuffer'> & {
        responseType: 'arraybuffer';
    }): Promise<ArrayBuffer>;
    (path: string, options: IApiOptions<'text'> & {
        responseType: 'text';
    }): Promise<string>;
    <T = unknown>(path: string, options?: IApiOptions & {
        responseType?: 'json';
    }): Promise<T>;
}
/** Overloaded signature for API methods with an optional request body. */
interface IApiWrite {
    (path: string, body: unknown, options: IApiOptions<'response'> & {
        responseType: 'response';
    }): Promise<Response>;
    (path: string, body: unknown, options: IApiOptions<'blob'> & {
        responseType: 'blob';
    }): Promise<Blob>;
    (path: string, body: unknown, options: IApiOptions<'arraybuffer'> & {
        responseType: 'arraybuffer';
    }): Promise<ArrayBuffer>;
    (path: string, body: unknown, options: IApiOptions<'text'> & {
        responseType: 'text';
    }): Promise<string>;
    <T = unknown>(path: string, body?: unknown, options?: IApiOptions & {
        responseType?: 'json';
    }): Promise<T>;
}
...
declare class IUserMenuStore {
    ...
    private generation;
    private disposed;
    constructor(currentUserService: ICurrentUserService, menuService: IUserMenuService, session: ISessionService, config?: IAuthConfig);
    subscribe: (listener: () => void) => (() => void);
    getVersion: () => number;
    ...
    loadAuthorizations(applicationId?: string): Promise<IEffectiveAuthorizationDto[]>;
...
declare class ISessionService {
    ...
    private storageKey;
    private invalidatePendingSession;
...
declare function createIRuntime(overrides: IAuthConfigOverrides): IRuntime;
...
declare function IAuthProvider(props: IAuthProviderProps): React$1.JSX.Element;
...
type IPermissionSource = 'menuCode' | 'role';
...
declare function evaluatePermission(value: IPermissionInput | null | undefined, source: IAuthorizationSource): boolean;
...
type IRouteMenuCodeResolver = (path: string, store: IUserMenuStore) => string | null | undefined;
 ...
 */
declare function IRequireRouteAccess({ menuCode, resolveMenuCode, missingMenuCode, unauthorizedPath, loading, children, }: IRequireRouteAccessProps): string | number | bigint | boolean | React$1.JSX.Element | Iterable<ReactNode> | Promise<string | number | bigint | boolean | React$1.ReactPortal | React$1.ReactElement<unknown, string | React$1.JSXElementConstructor<any>> | Iterable<ReactNode>>;

/** Keeps denied route content inside the current shell and URL. */
declare function IHasMnRoute({ value, children }: {
    value: IPermissionInput;
    children: ReactNode;
}): ReactElement;
/** Wraps an eager element or lazy route loader with reactive menu authorization. */
declare function hasMn(value: IPermissionInput, element: ReactNode): ReactElement;
declare function hasMn(value: IPermissionInput, loader: ILoadComponent): ILoadComponent;

export { DEFAULT_ERROR_FACTORIES, DEFAULT_PERSONAL_PROFILE_URL, HostShell, IAlert, type IAlertData, type IApiClient, type IApiClientDeps, type IApiError, type IApiErrorCatalogResolver, type IApiErrorDisplayFormatter, type IApiOptions, type IApiRead, type IApiWrite, IAuthCallback, type IAuthConfig, type IAuthConfigOverrides, IAuthContext, type IAuthEndpoints, IAuthProvider, type IAuthProviderProps, IAuthService, type IAuthUser, type IAuthorizationSource, IAvatar, type IAvatarProps, type IAvatarShape, type IAvatarSize, type IBreadcrumbItem, IButton, type IButtonProps, type IButtonSize, type IButtonType, type IButtonVariant, ICard, ICardBody, ICardFooter, ICardImage, type ICardProps, type ICodeHighlighter, ICodeViewer, type ICodeViewerFileLoaded, type ICodeViewerProps, IConfirm, type IConfirmData, ICsrfService, type ICurrentUserDto, ICurrentUserService, IDatepicker, type IDatepickerPanelPosition, type IDatepickerProps, IDialog, type IDialogAction, type IDialogActionCancel, type IDialogActionConfirm, type IDialogActionCustom, type IDialogActionOK, type IDialogActionObject, type IDialogActionSave, type IDialogActionType, type IDialogActionTypes, IDialogClose, type IDialogCloseProps, type IDialogConfig, IDialogContainer, type IDialogContainerProps, type IDialogInstance, IDialogOutlet, type IDialogProps, IDialogProvider, type IDialogProviderProps, IDialogRef, type IEffectiveAuthorizationDto, type IEffectiveAuthorizationType, type IEnvironment, type IErrorContext, IErrorPage, type IErrorPageAction, type IErrorPageKind, type IErrorPageMode, type IErrorPageProps, type IErrors, IFCDatepicker, type IFCDatepickerProps, IFCInput, type IFCInputProps, IFCSelect, type IFCSelectHandle, type IFCSelectProps, IFCTextArea, type IFCTextAreaProps, type IFavoriteMenuItemDto, type IFavoriteOrderItemDto, type IForgotPasswordResponse, type IFormControlErrorMessage, IGrid, IGridColumn, IGridColumnGroup, type IGridColumnGroupProps, type IGridColumnLike, type IGridColumnProps, type IGridColumnWidth, IGridCustomColumn, type IGridCustomColumnProps, IGridDataSource, type IGridDataSourceConfig, IGridExpandableRow, type IGridExpandableRowProps, type IGridExpandableRowRenderCtx, type IGridFilter, type IGridHandle, type IGridHeaderItem, type IGridPaginatorInput, type IGridProps, type IGridSelectionChange, type IGridSelectionMode, type IGridServerPage, type IGridServerSideConfig, type IGridSortAccessor, IHContent, IHContentLayout, IHMenu, IHSidebar, type IHSidebarProps, IHasMn, IHasMnRoute, type IHostApi, IHostApiProvider, IHostUiProvider, type IHostUiState, IIcon, type IIconInput, type IIconName, type IIconProps, type IIconSize, IInput, IInputAddon, type IInputAddonButton, type IInputAddonIcon, type IInputAddonKind, type IInputAddonLink, type IInputAddonLoading, type IInputAddonProps, type IInputAddonText, type IInputAddonType, type IInputAddons, type IInputMask, type IInputMaskType, type IInputProps, type IKnownErrorCode, type ILoadComponent, ILoading, type ILoadingProps, type ILoginResponse, type IMenu, type IMenuApplication, type IMenuApplicationDto, type IMenuCompany, type IMenuCompanyDto, type IMenuFavoriteReorderEvent, type IMenuFavoriteToggleEvent, type IMenuNodeDto, type IMenuOpenIn, type IMenuOpenInDto, type IMfaChallengeResponse, type INormalizedApiError, INotHasMn, IPaginator, type IPaginatorProps, type IPaginatorState, type IPermissionInput, type IPermissionPredicate, type IPermissionSource, IPill, type IPillProps, type IPillSize, type IPillVariant, type IQueryParams, type IRefreshResponse, type IRequestOptions, IRequireAccess, type IRequireAccessProps, IRequireAuth, IRequireRouteAccess, type IRequireRouteAccessProps, type IResetPasswordResponse, type IResponseType, type IRoute, type IRouteComponent, type IRouteMenuCodeResolver, IRouter, type IRouterProps, type IRoutes, type IRuntime, type ISanitizedReturnUrl, ISection, ISectionBody, ISectionFilter, ISectionFooter, ISectionHeader, ISectionSubHeader, ISectionTab, type ISectionTabBadge, type ISectionTabProps, ISectionTabs, type ISectionTabsHeight, type ISectionTabsProps, ISelect, type ISelectChange, type ISelectHandle, type ISelectPanelPosition, type ISelectProps, ISessionExpiredDialog, type ISessionExpiredReason, ISessionExpiredService, ISessionService, type ISessionUser, type ISortConfig, type ISortDirection, type ISortState, IStorageService, ITextArea, type ITextAreaProps, IToggle, type IToggleProps, type IToggleSize, type ITokenLifespan, type IUISize, type IUIVariant, type IUser, type IUserMenuEnvelopeDto, type IUserMenuLoadErrors, type IUserMenuLoadSource, IUserMenuService, IUserMenuStore, type IValidateResetTokenResponse, I_ERROR_PAGE_ACTIONS, I_ERROR_PAGE_PRESETS, I_ERROR_PAGE_SUPPORT_EMAIL, I_ICON_NAMES, I_ICON_SIZES, type ResolveControlErrorMessageArgs, type RouterLinkInput, UNAUTHORIZED_ACCESS_PATH, USER_APPLICATION_MAPPING_NOT_FOUND, type UseInputMaskOptions, asMinMaxLengthError, buildExternalSigninUrl, buildFavoritePathMap, buildUrl, collectAuthorizationScope, collectLeafRoutes, collectMenuChain, collectMenuCodes, createApiClient, createIRuntime, decodeJwtPayload, decodeUser, environment, evaluatePermission, extractAccessTokenFromHash, extractProblemDetailsErrorCode, findFirstLeafRoute, findMenuNameById, formatApiFieldErrors, getAuthEndpointPath, getAuthEndpointUrl, getDefaultIAuthConfig, getDefaultIAuthEndpoints, getMenuChildren, getMenuKey, getMenuLabel, getMenuRoute, hasAnyMenuCode, hasAnyRoute, hasMenuChildren, hasMn, hasNumber, interpolate, isControlRequired, isGroupNode, isHttpRoute, isLeafItem, isModuleMenu, isNewTabMenu, isRecord, isReloadMenu, isSessionExpiredError, isSpaMenu, mapToSidebarUser, normalizeApiError, normalizeFetchError, normalizeMenuTree, normalizeRoutePath, rawRequest, readNumber, requireIdentityHost, resolveApiErrorDisplayMessage, resolveControlErrorMessage, resolveIAuthConfig, sanitizeReturnUrl, sessionExpiredService, storageService, toIMenu, toIMenuFavorite, toIMenus, toSessionExpiredReason, useHostApi, useHostApiOptional, useHostUi, useIAlert, useIApi, useIAuth, useIAuthContext, useIConfirm, useICsrf, useIDialog, useIDialogData, useIDialogRef, useISession, useISessionExpired, useIUserMenuStore, useInputMask, useOptionalIConfirm, usePermission, validateIAuthConfig };
