...
import React8, { useRef, useState, useMemo, forwardRef, useEffect, useImperativeHandle, createContext, memo, useLayoutEffect, useCallback, useContext, Children, Suspense, lazy } from 'react';
import { useNavigate, Outlet, useLocation, Routes, Route, Navigate, useMatches } from 'react-router-dom';
...
function IIcon(props) {
  ...
  return /* @__PURE__ */ jsx("i-icon", { className, size, ...rest, children: /* @__PURE__ */ jsx("i", { className: innerClass }) });
}
function ILoading(props) {
  const { label = "Loading..", light = false, className, ...rest } = props;
  return /* @__PURE__ */ jsxs("i-loading", { className, light: light ? "" : void 0, ...rest, children: [
    /* @__PURE__ */ jsx(
      "div",
      {
        className: [
...
function IButton(props) {
  ...
  return /* @__PURE__ */ jsx(
    "i-button",
    {
      ...
      children: loading ? /* @__PURE__ */ jsx(ILoading, { label: loadingText, light: variant !== "outline" }) : /* @__PURE__ */ jsxs(Fragment, { children: [
...
function isDev() {
  var _a;
  try {
    return ((_a = import.meta.env) == null ? void 0 : _a.DEV) === true;
  } catch {
...
function ICard(props) {
  ...
  const relAttr = targetIsBlank ? mergeRel(rel ?? void 0, "noopener noreferrer") : rel ?? void 0;
  const hrefAttr = disabled ? void 0 : normalizedHref;
  const handleClick = (ev) => {
    ...
    if (hasClick) {
      ev.preventDefault();
      onCardClick == null ? void 0 : onCardClick(ev);
      return;
    }
    if (!hrefAttr) {
      ev.preventDefault();
  ...
  return /* @__PURE__ */ jsx("i-card", { ...hostProps, children: /* @__PURE__ */ jsx(
    "a",
    {
      className: "i-card",
      role: "link",
...
function ICodeViewer(props) {
  ...
  const [rawCode, setRawCode] = useState(() => (code ?? "").toString());
  useEffect(() => {
  ...
  return /* @__PURE__ */ jsx("i-code-viewer", { ...rest, children: /* @__PURE__ */ jsxs(
    "div",
    {
      className: [
        "i-code-viewer",
...
function IInputAddon(props) {
  ...
  return /* @__PURE__ */ jsx("i-input-addon", { className, kind: addon.type, ...rest, children: addon.type === "button" ? /* @__PURE__ */ jsx(
    IButton,
    {
      size: "xs",
      type: "button",
...
function useInputMask(inputRef, mask, opts = {}) {
  ...
  useEffect(() => {
    ...
    const formatDateSimple = (d, format) => {
      const day = d.getDate();
      const month = d.getMonth() + 1;
      const year = d.getFullYear();
      return format.replace(/yyyy/g, pad(year, 4)).replace(/MM/g, pad(month, 2)).replace(/dd/g, pad(day, 2));
    };
    const formatTimeSimple = (d, format) => {
      const hour = d.getHours();
      const minute = d.getMinutes();
      const second = d.getSeconds();
      return format.replace(/HH/g, pad(hour, 2)).replace(/mm/g, pad(minute, 2)).replace(/ss/g, pad(second, 2));
    };
    const computeDefaultForMask = () => {
      if (!mask) return null;
    ...
    const normalizeDateValue = (value, format) => {
      ...
      const maxDay = daysInMonth(year > 0 ? year : 2e3, month);
      if (day < 1) day = 1;
    ...
    const adjustDateSegmentByArrow = (key) => {
      ...
      if (month < 1) month = 1;
      if (month > 12) month = 12;
      let maxDay = daysInMonth(year > 0 ? year : 2e3, month);
      if (day < 1) day = 1;
      if (day > maxDay) day = maxDay;
    ...
    const onKeydown = (event) => {
      ...
      const type = mask.type;
      const key = event.key;
      if (type === "date" && mask.format && (key === "ArrowUp" || key === "ArrowDown")) {
        event.preventDefault();
        adjustDateSegmentByArrow(key);
        return;
      }
      if (type === "time" && mask.format && (key === "ArrowUp" || key === "ArrowDown")) {
        event.preventDefault();
...
var IInput = React8.forwardRef(
  function IInput2(props, forwardedRef) {
    ...
    return /* @__PURE__ */ jsxs("i-input", { className, onClick: handleHostClick, children: [
      prepends.map((a, idx) => /* @__PURE__ */ jsx(IInputAddon, { addon: a }, `pre-${idx}`)),
      /* @__PURE__ */ jsx(
        "input",
        {
          ref: setRefs,
      ...
      appends.map((a, idx) => /* @__PURE__ */ jsx(IInputAddon, { addon: a }, `app-${idx}`))
    ] });
  }
);
function IFCInput(props) {
  ...
  return /* @__PURE__ */ jsxs("i-fc-input", { ...hostProps, children: [
    label ? /* @__PURE__ */ jsxs("label", { className: "i-fc-input__label", children: [
      label,
      " :"
    ] }) : null,
    /* @__PURE__ */ jsx(
...
function highlightHtml(label, term) {
  const t = (term ?? "").trim();
  if (!t) return escapeHtml2(label);
  const safeLabel = escapeHtml2(label);
  const safeTerm = escapeHtml2(t);
...
function resolveDisplayText(row, rawOptions, displayWith, displayWithExplicit) {
  if (row === null) return "";
  if (typeof displayWith === "function" && displayWithExplicit) {
    return displayWith(row);
  }
  if (typeof displayWith === "string") {
    const v = getByPath(row, displayWith);
  ...
  if (!displayWithExplicit && (row === null || typeof row !== "object")) {
    ...
    if (match) {
      ...
      const labelEntry = entries[1] ?? entries[0];
      const labelValue = labelEntry == null ? void 0 : labelEntry[1];
      return labelValue !== null && labelValue !== void 0 ? String(labelValue) : String(primitive);
    }
  }
...
function computeFiltered(isOpen, filterText, rawOptions, filterPredicate) {
  if (!isOpen) return [];
...
var ISelect = forwardRef(function ISelectInner(props, ref) {
  const {
    ...
    loading = false,
    displayWith,
    filterPredicate = (row, term) => JSON.stringify(row).toLowerCase().includes(term),
    value = null,
    onChange,
    onChanged,
    onOptionSelected,
  ...
  const [modelValue, setModelValue] = useState(value ?? null);
  ...
  const displayWithExplicit = useMemo(
    () => displayWith !== void 0 && displayWith !== null,
    [displayWith]
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const valueToUse = modelValue !== null && modelValue !== void 0 ? modelValue : pendingModelValue;
    ...
    if (valueToUse === null || valueToUse === void 0) {
      setDisplayText("");
      setPendingModelValue(null);
    } else {
      const found = optionsArr.find((row2) => row2 === valueToUse) ?? null;
  ...
  const focus = () => {
    var _a;
    if (disabled) return;
    (_a = inputElRef.current) == null ? void 0 : _a.focus();
  };
  const openDropdown = () => {
    if (disabled) return;
    if (isOpen) return;
  ...
  const clearSelection = (ev) => {
    ...
    const payload = { value: null, label: "" };
  ...
  const selectRow = (row) => {
    ...
    setFilterText("");
    onChange == null ? void 0 : onChange(row);
      ...
      children: [
        ...
        hasOptionsList ? /* @__PURE__ */ jsx(
          "div",
          {
            ...
            ref: optionsListRef,
            children: filteredOptions.map((row, idx) => {
              ...
              return /* @__PURE__ */ jsxs(
                "div",
                {
                  ...
                  children: [
                    renderOption ? /* @__PURE__ */ jsx("div", { className: "i-option-label", children: renderOption(row) }) : /* @__PURE__ */ jsx(
                      "div",
                      {
                        className: "i-option-label",
                        dangerouslySetInnerHTML: {
                          __html: highlightSearch ? highlightHtml(label, filterText) : escapeHtml2(label)
                        }
                      }
                    ),
                    isSelected ? /* @__PURE__ */ jsx("span", { className: "i-option-check", children: /* @__PURE__ */ jsx(IIcon, { icon: "check" }) }) : null
                  ]
                },
                (row == null ? void 0 : row.id) ?? `${idx}-${label}`
              );
            })
          }
        ) : null
...
function IFCSelect(props) {
  ...
  return /* @__PURE__ */ jsxs("i-fc-select", { ...hostProps, children: [
    label ? /* @__PURE__ */ jsxs("label", { className: "i-fc-select__label", children: [
      label,
      " :",
      " ",
    ...
    invalid && resolvedErrorText ? /* @__PURE__ */ jsx("div", { className: "i-fc-select__error", children: resolvedErrorText }) : null
  ] });
}
...
function formatDateByFormat(date, fmt) {
  const yyyy = String(date.getFullYear()).padStart(4, "0");
  const MM = String(date.getMonth() + 1).padStart(2, "0");
  const dd = String(date.getDate()).padStart(2, "0");
  return (fmt || "dd/MM/yyyy").replace(/yyyy/g, yyyy).replace(/MM/g, MM).replace(/dd/g, dd);
}
function parseInputDate(value, fmt) {
...
function panelPositionClass2(panelPosition) {
  const value = (panelPosition || "bottom left").trim();
  const normalized = value.replace(/\s+/g, "-");
...
function IDatepicker(props) {
  ...
  const ensureYearRange = useCallback((focusYear) => {
    setYears((prev) => {
      if (prev.length && focusYear >= prev[0] && focusYear <= prev[prev.length - 1]) {
        return prev;
      }
      const start = focusYear - 50;
      const end = focusYear + 10;
  ...
  const mask = useMemo(() => ({ type: "date", format }), [format]);
  ...
  const onMonthChange = useCallback((change) => {
      ...
      children: [
        /* @__PURE__ */ jsx(
          IInput,
          {
            ...
            value: displayText,
            inputRef: inputElRef,
            onInput: (e) => handleTypedInput(e.target.value)
          }
        ),
        isOpen ? /* @__PURE__ */ jsxs(
          "div",
          {
            className: [
              "i-datepicker-panel",
              panelPositionClass2(panelPosition)
            ].join(" "),
            children: [
...
function IFCDatepicker(props) {
  ...
  return /* @__PURE__ */ jsxs("i-fc-datepicker", { ...hostProps, children: [
    label ? /* @__PURE__ */ jsxs("label", { className: "i-fc-datepicker__label", children: [
      label,
      " :",
      " ",
...
function resolveErrorMessage3(label, errorKey, errorMessage) {
  const tpl = errorMessage == null ? void 0 : errorMessage[errorKey];
  if (!tpl) return null;
  return tpl.replaceAll("{label}", label);
}
function ITextarea(props) {
  ...
  return /* @__PURE__ */ jsx("i-textarea", { children: /* @__PURE__ */ jsx(
    "textarea",
    {
      ...rest,
      "aria-invalid": invalid ? "true" : void 0,
...
function IFCTextarea(props) {
  ...
  const resolvedErrorText = useMemo(() => {
    if (!invalid) return null;
    return resolveErrorMessage3(label || "This field", errorKey, errorMessage) ?? `${label || "This field"} is invalid`;
  }, [invalid, label, errorKey, errorMessage]);
  return /* @__PURE__ */ jsxs("i-fc-textarea", { ...hostProps, children: [
    label ? /* @__PURE__ */ jsxs("label", { className: "i-fc-textarea__label", children: [
      label,
      " :",
      " ",
...
var DialogInstanceContext = createContext(
  null
);
function useIDialog() {
...
function IDialogProvider(props) {
  ...
  return /* @__PURE__ */ jsx(DialogContext.Provider, { value: api, children });
}
function IDialogContainer(props) {
  ...
  return /* @__PURE__ */ jsxs("i-dialog-container", { "data-dialog-id": instance.id, children: [
...
function isVisible(a) {
  if (!a) return false;
  return a.visible === void 0 ? true : !!a.visible;
}
function IDialog(props) {
  ...
  const hasAnyActions = customActions.some(isVisible) || isVisible(okAction) || isVisible(confirmAction) || isVisible(saveAction) || isVisible(cancelAction);
  return /* @__PURE__ */ jsxs("i-dialog", { ...rest, children: [
    title ? /* @__PURE__ */ jsx("h4", { className: "i-dialog-title", children: title }) : null,
    /* @__PURE__ */ jsx("div", { className: "i-dialog-content", children }),
    hasAnyActions ? /* @__PURE__ */ jsxs("div", { className: "i-dialog-actions", children: [
      customActions.filter(isVisible).map((a, idx) => /* @__PURE__ */ jsx(
        IButton,
        {
          icon: a.icon,
      ...
      (isVisible(okAction) || isVisible(confirmAction) || isVisible(saveAction) || isVisible(cancelAction)) && customActions.filter(isVisible).length > 0 ? /* @__PURE__ */ jsx("span", { className: "flex-fill" }) : null,
      isVisible(okAction) ? /* @__PURE__ */ jsx(
        IButton,
        {
          icon: "check",
          variant: "primary",
          className: okAction == null ? void 0 : okAction.className,
          onClick: () => onOk == null ? void 0 : onOk(),
          children: "OK"
        }
      ) : null,
      isVisible(confirmAction) ? /* @__PURE__ */ jsx(
        IButton,
        {
          icon: "save",
          variant: "primary",
          className: confirmAction == null ? void 0 : confirmAction.className,
          onClick: () => onConfirm == null ? void 0 : onConfirm(),
          children: "Confirm"
        }
      ) : null,
      isVisible(saveAction) ? /* @__PURE__ */ jsx(
        IButton,
        {
          icon: "save",
          variant: "primary",
          className: saveAction == null ? void 0 : saveAction.className,
          onClick: () => onSave == null ? void 0 : onSave(),
          children: "Save"
        }
      ) : null,
      isVisible(cancelAction) ? /* @__PURE__ */ jsx(
        IButton,
        {
          icon: "cancel",
          variant: "danger",
          className: cancelAction == null ? void 0 : cancelAction.className,
          onClick: (e) => {
            e.preventDefault();
...
// src/components/grid/grid-data-source.ts
...
function IPaginator(props) {
  ...
  return /* @__PURE__ */ jsx("i-paginator", { className: "i-paginator", children: /* @__PURE__ */ jsxs("div", { className: "i-paginator flex align-center gap-md flex-fill", children: [
...
function IGrid(props) {
  ...
  const HeaderCell = (p) => {
    ...
    return /* @__PURE__ */ jsxs(
      "i-grid-header-cell",
      {
        ...
        children: [
          /* @__PURE__ */ jsx("span", { className: "i-grid-header-cell__content", children: children2 }),
  ...
  return /* @__PURE__ */ jsxs("i-grid", { className: "i-grid", role: "table", children: [
    /* @__PURE__ */ jsxs("i-grid-viewport", { className: "i-grid-viewport", children: [
      effectiveHeaderItems.length ? /* @__PURE__ */ jsxs("i-grid-header-row", { className: "i-grid-header-row", role: "row", children: [
        !treeEnabled && hasExpandableRow && !(expandableRowDef == null ? void 0 : expandableRowDef.expandSingle) ? /* @__PURE__ */ jsx(
          HeaderCell,
          {
      ...
      renderedData.map((row, rowIndex) => {
        ...
        return /* @__PURE__ */ jsxs(React8.Fragment, { children: [
                      ...
                      Cell,
                      {
                        ...
                        children: /* @__PURE__ */ jsxs("span", { className: "i-grid-tree-inline", children: [
                          ...
                          col.cellDef ? renderCellDef(col.cellDef, row, {
                            ...
                            index: rowIndex,
                            column: col
                          }) : /* @__PURE__ */ jsx("span", { className: "i-grid-tree-text", children: col.fieldName ? highlightSearch ? highlightSearch(
                            String(
                  ...
                  return /* @__PURE__ */ jsx(
                    Cell,
                    {
                      ...
                      children: col.cellDef ? renderCellDef(col.cellDef, row, {
                        ...
                        index: rowIndex,
                        column: col
                      }) : /* @__PURE__ */ jsx("span", { className: "i-grid-cell__content", children: col.fieldName ? highlightSearch ? highlightSearch(
                        String(
                          (row == null ? void 0 : row[col.fieldName]) ?? ""
                        ),
                        currentFilterText
          ...
          hasExpandableRow && isRowExpanded(row) ? /* @__PURE__ */ jsx(
...
function highlightSearchHtml(text, rawTerm) {
  ...
  if (!term) return escapeHtml3(text ?? "");
  const safeText = text ?? "";
  ...
  while (i < safeText.length) {
    ...
    if (idx === -1) {
      out += escapeHtml3(safeText.slice(i));
      break;
    }
    out += escapeHtml3(safeText.slice(i, idx));
    out += `<span class="highlight-search">${escapeHtml3(
...
function IHContent(props) {
  ...
  return /* @__PURE__ */ jsxs("ih-content", { children: [
    /* @__PURE__ */ jsxs("div", { className: "ih-content-header", children: [
      /* @__PURE__ */ jsx("a", { className: "i-clickable", onClick: toggleSidebar, children: sidebarVisibility ? /* @__PURE__ */ jsx("img", { alt: "sidebar-left", src: "/svgs/sidebar-left.svg" }) : /* @__PURE__ */ jsx("img", { alt: "sidebar-right", src: "/svgs/sidebar-right.svg" }) }),
...
var IHMenu = memo(function IHMenu2(props) {
  ...
  const renderIndent = (level) => {
    ...
    return Array.from({ length: level }).map((_, i) => /* @__PURE__ */ jsx("span", {}, i));
  };
  ...
  if (!menu) return null;
  return /* @__PURE__ */ jsx("ih-menu", { children: /* @__PURE__ */ jsxs(
    "li",
    {
      className: [
      ...
      children: [
        +menu.menuTypeId === 2 ? /* @__PURE__ */ jsx("small", { children: /* @__PURE__ */ jsx(Highlighted, { text: menu.menuName, term: filter }) }) : +menu.menuTypeId === 3 ? hasChild ? /* @__PURE__ */ jsxs("div", { onClick: clickGroup, children: [
          menu.level > 0 ? renderIndent(menu.level) : null,
          /* @__PURE__ */ jsx("i", { className: menu.icon ?? "" }),
          /* @__PURE__ */ jsx("h6", { children: /* @__PURE__ */ jsx(Highlighted, { text: menu.menuName, term: filter }) }),
          /* @__PURE__ */ jsx(
            "i",
            {
        ...
        hasChild ? /* @__PURE__ */ jsx("ul", { className: +menu.menuTypeId === 3 ? menu.visibility ?? "" : "", children: (menu.child ?? []).map((m) => /* @__PURE__ */ jsx(
          IHMenu2,
          {
            menu: m,
            filter,
...
function IHSidebar(props) {
  ...
  return /* @__PURE__ */ jsxs("ih-sidebar", { className: !visible ? "hidden" : void 0, children: [
    /* @__PURE__ */ jsx("div", { className: "ih-sidebar-header", children: user ? /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsx("div", { className: "user-image", children: /* @__PURE__ */ jsx("img", { alt: "User Image", src: user.userImagePath }) }),
    ...
    /* @__PURE__ */ jsx("div", { className: "ih-sidebar-body scroll scroll-y", children: /* @__PURE__ */ jsx("ul", { children: filteredMenus.map((m) => /* @__PURE__ */ jsx(
      IHMenu,
      {
        menu: m,
        filter: menuFilter,
...
function normalizePath(path) {
  if (path === "" || path == null) return void 0;
  return path;
}
...
function buildBreadcrumbs(matches) {
  const items = [];
  ...
  return routes.map((r, idx) => {
    ...
    if (r.index) {
      return /* @__PURE__ */ jsx(
        Route,
        {
          index: true,
          element,
          handle: { title: r.title, breadcrumb: r.breadcrumb }
        },
        `idx-${idx}`
      );
    }
    return /* @__PURE__ */ jsx(
      Route,
      {
        ...
        element,
        handle: { title: r.title, breadcrumb: r.breadcrumb },
        children: ((_a = r.children) == null ? void 0 : _a.length) ? renderRouteTree(r.children) : null
      },
      `${path ?? "root"}-${idx}`
    );
  });
...
function IRouterMetaSync() {
  ...
  useEffect(() => {
    ...
    const { title } = readHandle(last == null ? void 0 : last.handle);
...
function IRouter(props) {
  ...
  return /* @__PURE__ */ jsxs(Suspense, { fallback: loading, children: [
    /* @__PURE__ */ jsx(IRouterMetaSync, {}),
    /* @__PURE__ */ jsxs(Routes, { children: [
      redirectIndexTo ? /* @__PURE__ */ jsx(Route, { index: true, element: /* @__PURE__ */ jsx(Navigate, { to: redirectIndexTo, replace: true }) }) : null,
      routeElements,
...
function ISection(props) {
  return /* @__PURE__ */ jsx("i-section", { ...props });
}
function ISectionHeader(props) {
  return /* @__PURE__ */ jsx("i-section-header", { ...props, children: /* @__PURE__ */ jsx("h4", { children: props.children }) });
}
function ISectionSubHeader(props) {
  return /* @__PURE__ */ jsx("i-section-sub-header", { ...props, children: /* @__PURE__ */ jsx("h6", { children: props.children }) });
}
function ISectionFilter(props) {
  return /* @__PURE__ */ jsx("i-section-filter", { ...props });
}
function ISectionBody(props) {
  return /* @__PURE__ */ jsx("i-section-body", { ...props });
}
function ISectionFooter(props) {
  return /* @__PURE__ */ jsx("i-section-footer", { ...props });
}
function ISectionTabHeader(props) {
  return /* @__PURE__ */ jsx(Fragment, { children: props.children });
}
ISectionTabHeader.displayName = "ISectionTabHeader";
function ISectionTabContent(props) {
  return /* @__PURE__ */ jsx(Fragment, { children: props.children });
}
...
function isElementOfType(el, comp) {
  return React8.isValidElement(el) && el.type === comp;
}
function resolveTab(tabEl, index) {
  ...
  const defaultHeader = /* @__PURE__ */ jsxs(Fragment, { children: [
  ...
  const defaultContent = /* @__PURE__ */ jsx(Fragment, { children: props.children });
  let headerNode = props.header ?? defaultHeader;
  let contentNode = props.content ?? defaultContent;
  if (props.children) {
    const childrenArr = React8.Children.toArray(props.children);
    const headerMarker = childrenArr.find(
      (c) => isElementOfType(c, ISectionTabHeader)
    );
    const contentMarker = childrenArr.find(
      (c) => isElementOfType(c, ISectionTabContent)
    );
    if (headerMarker) {
...
function ISectionTabs(props) {
  ...
  const tabs = useMemo(() => {
    const arr = React8.Children.toArray(children).filter(
      (c) => isElementOfType(c, ISectionTab)
    );
    return arr.map((t, i) => resolveTab(t, i));
  }, [children]);
  const isControlled = selectedIndex !== null && selectedIndex !== void 0;
  const initialIndex = useMemo(() => {
    if (isControlled && Number.isInteger(selectedIndex) && selectedIndex >= 0 && selectedIndex < tabs.length) {
  ...
  const activeIndexRaw = isControlled ? selectedIndex : uncontrolledIndex;
  const activeIndex = Number.isInteger(activeIndexRaw) && activeIndexRaw >= 0 && activeIndexRaw < tabs.length ? activeIndexRaw : tabs.length ? 0 : -1;
  ...
  const activate = useCallback(
    (idx) => {
      ...
      onSelectedIndexChange == null ? void 0 : onSelectedIndexChange(idx);
    },
    [tabs.length, isControlled, onSelectedIndexChange]
  );
  return /* @__PURE__ */ jsxs("i-section-tabs", { ...rest, children: [
    /* @__PURE__ */ jsx("div", { className: "i-section-tabs-headers", role: "tablist", children: tabs.map((tab, idx) => {
